...
public class FriendInfoActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titlebarLeftmenu = (Button) this
				.findViewById(R.id.main_titlebar_goback_leftmenu);
		titlebarRightmenu = (Button) this
				.findViewById(R.id.main_titlebar_gomore_rightmenu);
		headImage = (ImageView) this.findViewById(R.id.friendinfo_headimage);
		headImageBg = (ImageView) this
				.findViewById(R.id.friendinfo_headimage_bg);
		hometownButton = (Button) this
				.findViewById(R.id.friendinfo_hometown_button);
		signsButton = (Button) this.findViewById(R.id.friendinfo_signs_button);
		mobileButton = (Button) this
				.findViewById(R.id.friendinfo_mobile_button);
		...
		unfriendButton = (Button) popupView
				.findViewById(R.id.friendinfo_popupwindow_unfriend);
		unfriendButton.setOnClickListener(this);
		cancelButton = (Button) popupView
				.findViewById(R.id.friendinfo_popupwindow_cancel);
	...
	private void initFriendInfo(String qq) {
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				hometowntTextView.setText(friend.getProvince() + " "
						+ friend.getCity());
				...
				Bitmap headBg = ImageTool.getHeadImageBgFromLocalOrNet(
						getApplicationContext(), User.qq);
				headImageBg.setBackgroundDrawable(new BitmapDrawable(headBg));
				Bitmap bitmap = ImageTool.getHeadImageFromLocalOrNet(
						getApplicationContext(), qq);
	...
	public void onClick(View v) {
		...
		case R.id.addfriend_button:
			if (tag != null && tag.equals("agreeRequest")) { // 表示是同意好友添加请求
				try {
					oos = new ObjectOutputStream(ManageClientConnServer
							.getClientConServerThread(User.qq).getS()
							.getOutputStream());
					...
					String time = TimeConvertTool.convertToString(new Date(
		...
		case R.id.friendinfo_popupwindow_unfriend:
			...
			try {
				...
				msg.setSender(User.qq);
				msg.setReceiver(qq);
				String time = TimeConvertTool.convertToString(new Date(System
						.currentTimeMillis()));
				msg.setTime(time);
				oos.writeObject(msg);
			} catch (Exception e) {
